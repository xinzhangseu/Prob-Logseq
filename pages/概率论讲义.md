...
3. 如果R和S是可测集合，并且R c S，则S—R也是可测集合。此外，m(S—R)=m(S)—m(R)。
4. The measure of a bounded interval is the length of that interval (whether open, closed, or half open).
5. The measure of a countable union of pairwise disjoint measurable sets is the sum of their measures.
6. If $R$ and $S$ are measurable sets, $R \subseteq S$, then so is $S-R$. Furthermore, $m(S-$ $R)=m(S)-m(R)$.
