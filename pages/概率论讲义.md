...
>  一个区间是 $\mathbb{R}$ 的子集，其形式为 $[a, b]:=\{x \in \mathbb{R}: a \leq x \leq b\},[a, b):=\{x \in \mathbb{R}: a \leq x<b\},(a, b]:=\{x \in \mathbb{R}: a<x \leq b\}$，或者  $(a, b):=\{x \in \mathbb{R}:$ $a<x<b\}$，其中， $a \leq b$ 是实数。我们定义区间 $I=[a, b],[a, b),(a, b],(a, b)$ 的长度 $|I|:=b-a$。A box in $\mathbf{R}^{d}$ is a Cartesian product $B:=I_{1} \times \ldots \times I_{d}$ of $d$ intervals $I_{1}, \ldots, I_{d}$ (not necessarily of the same length), thus for instance an interval is a one-dimensional box. The volume $|B|$ of such a box $B$ is defined as $|B|:=\left|I_{1}\right| \times \ldots \times\left|I_{d}\right|$. An elementary set is any subset of $\mathbf{R}^{d}$ which is the union of a finite number of boxes.
