...
> 2.  称 $B$ 是 $\mathbb{R}^d$ 空间上的矩形, 如果 $B\sub\{ \mathbb{R} \}$ A box in $\mathbf{R}^{d}$ is a Cartesian product $B:=I_{1} \times \ldots \times I_{d}$ of $d$ intervals $I_{1},  \ldots,  I_{d}$ (not necessarily of the same length),  thus for instance an interval is a one-dimensional box. The volume $|B|$ of such a box $B$ is defined as $|B|:=\left|I_{1}\right| \times \ldots \times\left|I_{d}\right|$. An elementary set is any subset of $\mathbf{R}^{d}$ which is the union of a finite number of boxes.
