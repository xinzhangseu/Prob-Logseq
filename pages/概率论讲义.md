...
> [!math|{"type":"definition","number":"auto","setAsNoteMathLink":false,"label":"jordan-measure","_index":1}] 定义 2.
> 	(约当测度) 令 $E$ 为 $\mathbb{R}^d$ 的有界子集。
> 	1.  称 $m_{*, J}(E):=\sup_{A\subset E, A\mbox{为基本集}}m (A)$ 为约当内测度;
		1. 称 $m^{*, J}(E):=\inf_{A\supset E, A\mbox{为基本集}}m (A)$ 为约当外测度;
		2. 称 $E$ 为约当可测的, 如果 $m_{*,J}(E)=m^{*,J}(E)$. 此时称 $m(E):=m_{*,J}(E)=m^{*,J}(E)$ 为集合 $E$ 的约当测度.
