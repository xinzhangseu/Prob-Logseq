...
A similar set for which the same argument should apply is Volterra's set, SVC (4). Its complement in $[0,1]$ is a union of pairwise disjoint intervals whose combined length is $1 / 2$. We know that the outer content of $\operatorname{SVC}(4)$ is $1 / 2$, but since this set contains no open intervals, its inner content is 0 . It is not Jordan measurable. Borel believed that measure should be redefined so that $\mathrm{SVC}(4)$ has a well-defined measure equal to $1 / 2$.

The problem with Jordan measure is that it is only finitely additive. Borel realized that he needed a measure that is countably additive. This means that if we are given
5.1 Peano, Jordan, and Borel
127
an infinite sequence of pairwise disjoint sets, $\left(S_{1}, S_{2}, S_{3}, \ldots\right), S_{i} \cap S_{j}=\emptyset$ for $i \neq j$, then we want the measure of the union to equal the sum of the measures,
$$
m\left(\bigcup_{k=1}^{\infty} S_{k}\right)=\sum_{k=1}^{\infty} m\left(S_{k}\right) .
$$
As an example, such a countably additive measure would imply that the set of rational numbers in $[0,1]$, a countable union of single points, must have measure 0 . Borel begins with three assumptions that uniquely define Borel measure:
1. The measure of a bounded interval is the length of that interval (whether open, closed, or half open).
2. The measure of a countable union of pairwise disjoint measurable sets is the sum of their measures.
3. If $R$ and $S$ are measurable sets, $R \subseteq S$, then so is $S-R$. Furthermore, $m(S-$ $R)=m(S)-m(R)$.

As an example, each $\operatorname{set} \operatorname{SVC}(n), n \geq 3$, is the complement in $[0,1]$ of a countable union of open intervals. It is measurable in Borel's sense, and its measure is $(n-3) /(n-2)$. In fact, any open set is a countable union of open intervals, so open sets and closed sets are measurable in Borel's sense.
