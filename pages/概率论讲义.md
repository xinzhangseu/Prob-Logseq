...
> 2.  称 $B$ 是 $\mathbb{R}^d$ 空间上的矩形, 如果 $B\subset\mathbb{R}^d$ 且可以写成 $d$ 个区间 $I_{1}, I_{2}, \cdots, I_d$ 的笛卡尔积, 即 $B:=I_{1} \times \cdots \times I_{d}$  (不一定要求区间有相同的长度). 我们定义矩形 $B$ 的体积  $|B|:=\left|I_{1}\right| \times \cdots \times\left|I_{d}\right|$. 
> 3. 称 $C$ 是 $\mathbb{R}^d$ 上的基本集, 如果它是 $\mathbb{R}^d$ 子集且可以写成有限矩形的 An elementary set is any subset of $\mathbf{R}^{d}$ which is the union of a finite number of boxes.
