...
对于 [a,b] 上的正值连续函数 f(x)，其 Riemann 积分的几何意义是其下方曲边梯形的面积，因此 Riemann 积分的定义以及一个函数的可积性，是与相应的下方图形面积如何确定以及是否存在密切相关。

  

**也就是说 Riemann 不可积函数，就反映为其下方曲边梯形的面积不存在**。这会导致很多局限性。

  

为了建立能够适用于更大函数类的新的积分理论，那么一个自然的想法就是推广原来的面积的概念，也就是说希望对一般 $\mathbb R^n$\mathbb R^n 中的点集 E 指定一个**非负的数 m(E)，它是长度、面积或体积的推广，希望能够满足几个直观的条件**：

  

（1） $m(E) \geq 0$m(E) \geq 0

  

（2）当 A 和 B 不相交时有 $m(A \bigcup B)=m(A)+m(B)$m(A \bigcup B)=m(A)+m(B) ，当 $A \subseteq B$A \subseteq B 时有 $m(A) \leq m(B)$m(A) \leq m(B)
...
（4）平移不变性 $\forall x \in \mathbb R^n, m(x+A)=m(A)$\forall x \in \mathbb R^n, m(x+A)=m(A)
...
**外测度定义：**设 E 是 $\mathbb R^n$\mathbb R^n 的点集，若 ${I_k}_{k=1}^\infty${I_k}_{k=1}^\infty 是 E 中的一列开矩体，且是 E 的一个覆盖，则它确定了一个非负实数

  

$u=\sum_{k=1}^\infty |I_k|$u=\sum_{k=1}^\infty |I_k|
