...
例如，每个形如 $\operatorname{SVC}(n), n \geq 3$的集合都是在[0, 1]中一组开区间取补得到的。按照 Borel 意义上来说它们都是可以测量的，并且其度量为(n —3)/(n—2)。实际上，任何开放集都可以看作是一组开区间取并得到的，并且根据 Borel 意义上来说开放集和闭合集都可以进行测量。As an example, each  is the complement in $[0,1]$ of a countable union of open intervals. It is measurable in Borel's sense, and its measure is $(n-3) /(n-2)$. In fact, any open set is a countable union of open intervals, so open sets and closed sets are measurable in Borel's sense.
