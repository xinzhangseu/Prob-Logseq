...
Borel 非常接近我们现代的测度概念，但在他对其应用进行讨论时，仅限于使用可数并集和补集构建的集合. 今天，我们称这种方式构建的集合为 Borel 集. 。你需要证明（参见练习5.1.13-5.1.15），根据这个定义，所有开区间和半开区间都是 Borel 集合，任何可数个 Borel 集合的交集也是一个 Borel 集合
