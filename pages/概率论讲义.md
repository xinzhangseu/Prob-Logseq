...
$$
作为一个例子，这样一个可数可加的测度意味着在 $[0,1]$ 中有理数的集合，即单点的可数并集，必须具有度量为0As an example, such a countably additive measure would imply that the set of rational numbers in , a countable union of single points, must have measure 0 . Borel begins with three assumptions that uniquely define Borel measure:
