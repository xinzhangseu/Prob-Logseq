...
> 2. 如果将 $E$ 划分为有限个不相交基本集的并集  $B_1\cup \cdots B_k$, 则  $m(E) :=|B_1|+\cdots+|B_k|$ 与划分无关. 换句话说, 对于任何其他不相交的划分  $B_1^\prime\cup \cdots \cup B_{k'}^\prime$ , 都有 $|B_1|+\cdots+|B_k|=|B_1^\prime+\cdotsB_{1} ′ 1 | + . . . + | B ′ k ′ |$ 。
