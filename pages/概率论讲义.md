...
#### 1.3.2.3 Borel 测度
 ...
 对于 Volterra 集合 SVC (4), 它在 $[0,1]$ 中的补集是一组两两不相交区间的并集，其总长度为 $1 / 2$. 我们知道 $\operatorname{SVC}(4)$ 的 Jordan 外测度为 $1 / 2$, 但由于该集合不包含开区间，其 Jordan 内测度为 $0$. It is not Jordan measurable. Borel believed that measure should be redefined so that $\mathrm{SVC}(4)$ has a well-defined measure equal to $1 / 2$.
