...
> 1. 称 $I$ 是 $\mathbb{R}$ 上的区间, 如果它是 $\mathbb{R}$ 的子集, 且其形式为 $[a,  b]:=\{x \in \mathbb{R}: a \leq x \leq b\}, [a,  b):=\{x \in \mathbb{R}: a \leq x<b\}, (a,  b]:=\{x \in \mathbb{R}: a<x \leq b\}$, 或者 $(a,  b):=\{x \in \mathbb{R}:$ $a<x<b\}$, 其中,  $a \leq b$ 是实数. 我们定义区间 $I=[a,  b], [a,  b), (a,  b], (a,  b)$ 的长度 $|I|:=b-a$. 
> 2.  称 $B$ 是 $\mathbb{R}^d$ 空间上的矩形, 如果 $B\subset\mathbb{R}^d$ 且可以写成 $d$ 个区间 $I_{1}, I_{2}, \cdots, I_d$ 的tm A box in $\mathbf{R}^{d}$ is a Cartesian product $B:=I_{1} \times \ldots \times I_{d}$ of $d$ intervals $I_{1},  \ldots,  I_{d}$ (not necessarily of the same length),  thus for instance an interval is a one-dimensional box. The volume $|B|$ of such a box $B$ is defined as $|B|:=\left|I_{1}\right| \times \ldots \times\left|I_{d}\right|$. An elementary set is any subset of $\mathbf{R}^{d}$ which is the union of a finite number of boxes.

#### 1.3.2.2 **Peano 和 Jordan 容度**

> [!math|{"type":"definition", "number":"auto", "setAsNoteMathLink":false, "label":"", "_index":2, "title":"约当测度"}] 定义 3 (约当测度).
> 令 $E$ 为 $\mathbb{R}^d$ 的有界子集, 则 1. 称 $m_{*,  J}(E):=\sup_{A\subset E,  A\ \mbox{\scriptsize 为基本集}}m (A)$ 为约当内测度;   2. 称 $m^{*,  J}(E):=\inf_{A\supset E,  A\ \mbox{\scriptsize 为基本集}}m (A)$ 为约当外测度;   3. 称 $E$ 为约当可测的, 如果 $m_{*, J}(E)=m^{*, J}(E)$. 此时称 $m(E):=m_{*, J}(E)=m^{*, J}(E)$ 为集合 $E$ 的约当测度. 
